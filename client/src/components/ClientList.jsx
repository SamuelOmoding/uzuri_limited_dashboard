...
const ClientList = React.memo(({ ThemeStyles }) => {
  ...
  useEffect(() => {
    fetch(`https://uzuri-limited-backend-veim.onrender.com/api/admin/routes/clients?page=${page}&limit=10`)
      .then(response => {
        if (!response.ok) {
    ...
    >
      ...
      {hasMore && (
        <button 
          ...
          className="py-2 px-3 mt-4 bg-gray-500 text-white font-bold rounded-full hover:bg-gray-900"
        >
          Load More
        </button>
      )}
