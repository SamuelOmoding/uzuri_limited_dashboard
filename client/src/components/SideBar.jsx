  ...
  return (
    <Link to={to}>
      <MenuItem className="text-black hover:text-gray-600" icon={icon}>
        <h4 className="text-lg md:text-xl text-gray-400 hover:text-gray-600">{title}</h4>
      </MenuItem>
    </Link>
  );
};
              ...
              >
                {!isCollapsed && (
                  <Box className="flex justify-between p-3">
                    <h1 className={`text-xl md:text-2xl font-bold ${darkTheme ? 'text-gray-400' : 'text-gray-900'}`}>UZURI LIMITED</h1>
                    <IconButton
            ...
            {!isCollapsed && (
              <Box>
                <Box className="mt-3 text-center">
                  {user ? (
                    <>
                      <h1 className={`text-xl md:text-2xl font-bold ${darkTheme ? 'text-white' : 'text-black'}`}>
                        {user.name}
                      </h1>
                      <h5 className={`text-lg md:text-xl font-bold ${darkTheme ? 'text-gray-400' : 'text-gray-900'}`}>
                        {user.role || "Admin"}
                      </h5>
                    </>
                  ) : (
